...
# Add custom rational imports
from modules.custom import (
    RationalWrapper,
    inject_rational_into_unet,
    freeze_base_model
)
...
def inject_rational_into_unet(unet, target_modules=["ff.net.0", "ff.net.2"], 
                               numerator_degree=3, denominator_degree=2):
    """Replace feedforward layers with rational activations"""
    replaced_count = 0
    ...
    for name, module in unet.named_modules():
        if any(target in name for target in target_modules):
            if isinstance(module, nn.Linear):
                ...
                # Create rational wrapper
                rational_wrapper = RationalWrapper(
                    pretrained_layer=module,
                    numerator_degree=numerator_degree,  # Use arg
                    denominator_degree=denominator_degree,  # Use arg
                    num_groups=1,
                    init_method='identity',  # Start closer to identity
                    freeze_linear=True
                )
                
                setattr(parent_module, child_name, rational_wrapper)
                replaced_count += 1
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,  # Start with lower degree for stability
        help="Numerator degree for rational activation",
    )
    parser.add_argument(
        "--rational_denominator_degree", 
        type=int,
        default=2,  # Lower degree
        help="Denominator degree for rational activation",
    )
...
def main(args):
    ...
    logger.info("Applying Rational Activation Functions...")
    unet = inject_rational_into_unet(
        unet,
        target_modules=["ff.net.0", "ff.net.2"],
        numerator_degree=args.rational_numerator_degree,
        denominator_degree=args.rational_denominator_degree
    )
    unet = freeze_base_model(unet)
